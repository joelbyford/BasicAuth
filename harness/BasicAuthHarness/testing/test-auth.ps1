...
curl -i -X POST "$BaseUrl/bogus" -H "X-Forwarded-Proto: https"

Write-Host "\n2) Bad credentials -> expect 401"
curl -i -X POST "$BaseUrl/bogus" -H "X-Forwarded-Proto: https" -H "Authorization: Basic $badAuthValue"

Write-Host "\n3) Valid credentials + bogus endpoint -> expect 404"
curl -i -X POST "$BaseUrl/bogus" -H "X-Forwarded-Proto: https" -H "Authorization: Basic $authValue"

Write-Host "\n4) Valid credentials + valid endpoint -> expect 200"
curl -i "$BaseUrl/health" -H "X-Forwarded-Proto: https" -H "Authorization: Basic $authValue"
